...
REM ============================================================
REM Выгрузка расширения конфигурации из базы в файл .cfe
REM
REM Параметры:
REM   %1 - путь к результирующему файлу .cfe
...
if "%~2"=="" (
    echo Использование: dump-cfe.bat ^<CFE_FILE^> ^<EXT_NAME^>
    echo.
    echo Пример: dump-cfe.bat "build\МоёРасширение.cfe" "МоёРасширение"
    exit /b 1
)
...
echo Выгрузка расширения в файл...
echo   Результат: %CFE_FILE%
echo   Расширение: %EXT_NAME%

"%ONEC_PATH%" DESIGNER !IB_PARAMS! !AUTH_PARAMS! /DisableStartupDialogs /DumpCfg "%CFE_FILE%" -Extension "%EXT_NAME%"
